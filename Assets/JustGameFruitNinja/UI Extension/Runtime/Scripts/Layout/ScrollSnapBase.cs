...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        internal void OnCurrentScreenChange(int currentScreen)
        {
            ...
            OnLastScreen(currentScreen);
        }


        private void OnLastScreen(int targetScreen)
        {
            //If this is using an Infinite Scroll, then don't disable
            if (!_isInfinite)
            {
                SkipButton.SetActive(targetScreen < _screensContainer.transform.childCount - 1);
                PlayButton.SetActive(targetScreen == _screensContainer.transform.childCount - 1);
            }
        }
